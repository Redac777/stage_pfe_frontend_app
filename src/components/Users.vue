                            ...
                            <template v-slot:activator="{ props }">
                                <div class="addActions">
                                    ...
                                    <div class="addUserButtonStyle">
                                        <v-btn icon v-bind="props" @click="openDialog" color="white"
        ...
        importUsers(reader, file) {
            reader.onload = (e) => {
                ...
                for (const row in jsonData) {
                    const roleId = this.getRoles.find((role) => role.name == jsonData[row][4]);
                    const profileGroupId = this.getProfileGroups.find((profileGroup) => profileGroup.type == jsonData[row][5]);
                    const shiftId = this.getShifts.find((shift) => shift.category == jsonData[row][6]);
                    if (row != 0) {
                        const user = {
                            ...
                            role_id: roleId.id,
                            profile_group_id: profileGroupId.id,
                            shift_id: shiftId.id
                        };
